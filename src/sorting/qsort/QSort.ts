...
export class QSort extends Sort {
    ...
    private static partition<T>(data: T[], low: number, heigh: number, comparator: TComparator): number {
        let pivot = low;
    ...
    private static qsort<T>(data: T[], low: number, heigh: number, comparator: TComparator) {     
        if(low < heigh){
